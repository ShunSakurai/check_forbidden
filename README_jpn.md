...
## 説明
memoQでは、禁止されている原語と訳語のペアを検証することはできますが、**禁止されている訳語のみを**、対応する原語に関係なく検証することはできません。これが不便だと思い、このツールを開発しました。
...
- 結果はファイルごとと、ファイル全体について(「Summary」(まとめ)として)表示されます
...
### オプション
検索の対象とするセグメントの種類を指定できます。オプション画面を表示するには、歯車アイコンをクリックします。
...
### CSVファイルの形式
各項目は、UTF-8でエンコードし、コンマで区切り、次のいずれかの形式にしてください。
...
- Run! - Enterキーまたはスペースキー
- オプションの表示/非表示 - oキー

## 既知の問題と対処法

### コマンドプロンプトの文字化け
Windowsのコマンドプロンプトでマルチバイト文字が文字化けしているように見える場合があります。タイトルバーを右クリックし、「プロパティ/フォント」を選択し、別のフォントを選択すると、この問題が解決します。
...
- バイリンガルファイル欄に入力または貼り付けする場合も、エクスポートする結果ファイルのパスとファイル名候補を表示
- 検索結果の上部に、CSVファイルの名前や使用したオプションに関する情報を表示
